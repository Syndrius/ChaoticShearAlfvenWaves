"""
This time we add a single resonant island and see what happens
In both continuum and full case, the matrix is not positive definite,
Matrices are not Hermitian, continuum looks good except for at the island, probably a resolution issue/number of surfaces issue.
Full spectrum looks similar, bit of a disaster around the sepratrix. Outside looks pretty good
"""
#%%
using Revise
using MID
...
#should be arbitrary!
#perhaps we stay in cylinder for this case!
R0=10.0

#amp needs further thought!
#define the non-resonant island
isl = IslandT(m0=5, n0=-2, A=0.0005)

geo = GeoParamsT(R0=R0)

#to stop the matrices from being trated as Hermitian
flr = FLRT(δ=1e-18)
prob = init_problem(q=qfm_benchmark_q, met=cylindrical_metric!, geo=geo, isl=isl, flr=flr)
...
#at a depth of 5, the 18th surface seems impossible to find, perhaps that is inside the island?
#depth of 3, all 8 are found in ~12s
#depth of 4, all 8 are found in ~63s
...
save_object("data/qfm/isl_surfs.jld2", surfs)

#%%

surfs = load_object("data/qfm/isl_surfs.jld2");
