...
isl32a = init_island(m0=3, n0=-2, w=0.05, r0=0.5, qp=2.0)
#start with no islands
prob = init_problem(geo=geo, q=MID.Equilibrium.island_equiv_q, met=:cylinder, isl=isl32a)
#%%
#so this q profile diverges at r=1.0, not ideal
rvals = LinRange(0.0, 1.0, 100)
qvals = [prob.q(i)[1] for i in rvals]
plot(rvals, qvals, legend=false)
#%%
Ntraj = 150;
...
#%%
#this part is cooked af now.
#probably need a second split between 0.8 and 0.85 or something tbh
#there is just an absurd amount of shear going on towards the edge.
rats1 = lowest_rationals(1, prob.q(0.8)[1], prob.q(0.97)[1])
...
save_object("/Users/matt/phd/MID/data/surfaces/island_32a.jld2", curr_surfs)
...
#jac is still pretty cooked, this may just be because of the R0 value. 
#we may need to change the R0 value, which could be a bit annoying.
#tis possible to just change Zhisongs code to use a cylindrical metric instead?
#probably easiest
#changing R0 results in a significantly smaller dJdr, but a larger B^s, unsure if that is a problemo or not!
