#same as 21a island, but we are now using flux coordinates to see if that helps fix things up a bit!
using MID
using MIDViz
using Statistics
using JLD2
...
isl21a = init_island(m0=2, n0=-1, w=0.1, ψ0=0.5, qp=2.0)
#start with no islands
prob = init_problem(geo=geo, q=MID.Equilibrium.flux_island_21_q, met=:cylinder, isl=isl21a)
display(prob.isls[1])

#MID.Geometry.inst_island(prob.isls[1], prob.q)
#%%
rvals = LinRange(0.0, 1., 100)
qvals = [prob.q(i)[1] for i in rvals]
plot(rvals, qvals)
#%%
Ntraj = 150;
rlist = collect(LinRange(0.05, 0.99, Ntraj));
...
rats3 = lowest_rationals(9, prob.q(0.4)[1], prob.q(0.6)[1])
...
plot_surfs(surfs4)
#%%
rats5 = lowest_rationals(11, prob.q(0.0)[1], prob.q(0.2)[1])
...
save_object("/Users/matt/phd/MID/data/surfaces/flux_island_21a.jld2", curr_surfs)
...
ψgrid_jac = init_grid(type=:rf, N = 100, start=0.03, stop=0.97)
θgrid_jac = init_grid(type=:af, N = 20) 
ζgrid_jac = init_grid(type=:af, N = 4)
grids_jac = init_grids(ψgrid_jac, θgrid_jac, ζgrid_jac)
B, jac, djac = compute_jac(prob, grids_jac, curr_surfs);
#%%
ψgrid_plot, θgrid_plot, ζgrid_plot = MID.Structures.inst_grids(grids_jac);

contourf(θgrid_plot, ψgrid_plot, jac[:, :, 1], levels=50, title="Jacobian")
contourf(θgrid_plot, ψgrid_plot, djac[1, :, :, 1], levels=50, title="dJdr")
contourf(θgrid_plot, ψgrid_plot, djac[2, :, :, 1], levels=50, title="dJdt")
#contourf(θgrid_plot, rgrid_plot, B[1, :, :, 1], levels=50, title="B^s")
contourf(θgrid_plot, ψgrid_plot, (B[1, :, :, 1]).^2, levels=50, title="B^s^2")
minimum(jac)
#%%
Nψ = ψgrid_jac.N
Bmean = zeros(Nψ);
...
for i in 1:Nψ
    ...
    Bsum[i] = sum((B[1, i, :, :]) .^2)
    djacψmean[i] = mean((djac[1, i, :, :]) .^2)
    djacψmax[i] = maximum(djac[1, i, :, :])
    djacψsum[i] = sum((djac[1, i, :, :]) .^2)
    djacθmean[i] = mean((djac[2, i, :, :]) .^2)
    djacθmax[i] = maximum(djac[2, i, :, :])
    djacθsum[i] = sum((djac[2, i, :, :]) .^2)
end
#%%
plot(ψgrid_plot, Bmean, title="B")
plot(ψgrid_plot, Bmax, title="Bmax")
plot(ψgrid_plot, Bsum, title="Bsum")
plot(ψgrid_plot, djacrmean, title="dJdrmean")
plot(ψgrid_plot, djacrmax, title="dJdrmax")
plot(ψgrid_plot, djacrsum, title="dJdrsum")
plot(ψgrid_plot, djacθmean, title="dJdtmean")
plot(ψgrid_plot, djacθmax, title="dJdtmax")
plot(ψgrid_plot, djacθsum, title="dJdtsum")
#%%
