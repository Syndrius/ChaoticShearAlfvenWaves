...
struct OldCoefficientsT
    ...
    function OldCoefficientsT(N::Int64)
...
struct OldActionGradInputsT
    ...
    rdot :: Array{Float64}
    θdot :: Array{Float64}
    function OldActionGradInputsT(p::Int64, q::Int64, Ntor::Int64, fft_size::Int64, ζ::StepRangeLen{Float64}, prob::ProblemT, met::MetT, B::BFieldT, nlist::UnitRange{Int64})
...
struct OldFTDataT
    ...
    function OldFTDataT(fft_size::Int64)
...
struct OldActionGradInputsJMT
    ...
    function OldActionGradInputsJMT(fft_size::Int64, ip::ActionGradInputsT)
...
function old_action(p::Int64, q::Int64, prob::ProblemT, met::MetT, B::BFieldT, MM::Int64, M::Int64, N::Int64, sguess=0.5::Float64)#, sguess::Float64, MM::Int64, M::Int64, N::Int64)
    ...
    ip = OldActionGradInputsT(p, q, Ntor, size_of_fft_array, ζ, prob, met, B, nlist)

    #creates a struct with preallocated memeory for fourier data.
    ftd = OldFTDataT(size_of_fft_array)

    ipjm = OldActionGradInputsJMT(size_of_fft_array, ip)

    #struct storing the actual coefficients to be found.
    coefs = OldCoefficientsT(qN+1)
    ...
    for jpq in 0:fM-1
        ...
        old_pack_coeffs!(x0, coefs, Ntor)

        #creates a standin function that fits the form needed for NLSolve.
        ag!(δS, x) = old_action_grad!(δS, x, a, coefs, ip, ftd)

        #creates a standin function that fits the form needed for NLSolve.
        #this function defines the gradient, improving efficiency.
        ag_JM!(JM, x) = old_action_grad_jm!(JM, x, a, coefs, ip, ftd, ipjm)
        ...
        old_unpack_coeffs!(sol.zero, coefs, Ntor)
