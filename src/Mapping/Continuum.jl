...
function tor_to_isl_continuum(dir_base::String, islgrids::MapGridsT)
    ...
    og_prob, grids = inputs_from_file(dir=dir_base);

    #fkn disaster, connection between fortran and julia is slipping further apart.
    if isnan(og_prob.isl.w) || isnan(og_prob.isl.A)
        display("island not initiated.")
        og_isl = IslandT(2, -1, 5.625e-5, 2.0, 2.0, 0.5, 0.03)
        #isl = inst_island(og_prob.isl, og_prob.q)
        #because our fortran read inputs function is not finished...
        #note that this name is different to the fortran one...
        #isl = inst_island(og_prob.isl, island_mode_21)
        prob = TorProblemT(q=island_mode_21, compute_met=og_prob.compute_met, dens=og_prob.dens, isl=og_isl, isl2=og_prob.isl2, flr=og_prob.flr, geo=og_prob.geo)
    else
        prob = og_prob
    end
...
#this will be the inverse mapping.
#ie taking results from inside the island
#and sussing what this will look like in our normal coords.
function isl_to_tor_continuum(dir_base::String, torgrids::MapGridsT)
    #perhaps we should allow using MID's solutions, but this probably requires parallel to ever be useful.
    ...
    rgrid, θgrid, ζgrid = inst_grids(torgrids)

    #probably wont actually be this size.
    rms = [] #Array{Float64}(undef, length(isl_ω))
    mode_labs = Tuple{Int, Int}[] 

    isl_ω = []

    κgrid, ᾱgrid, φgrid = inst_grids(grids)

    #the names are unfor in that they are cooked.
    rmarray = Array{Int64}(undef, torgrids.Nᾱ, torgrids.Nφ)
    ϕmarray = Array{Float64}(undef, torgrids.Nᾱ, torgrids.Nφ)
    #κmarray = Array{Int64}(undef, islgrids.Nᾱ-1, islgrids.Nφ)
    #ϕmarray = Array{Float64}(undef, islgrids.Nᾱ-1, islgrids.Nφ)
    ...
    for i in 1:1:length(evals.ω)
        ...
        #sign shouldn't matter here, as I think we are just doing κ < 1 atm.
        #will be subject to change. Just like the sign part lol.
        ϕ_tor, ϕ_torfft = isl_to_tor(torgrids, ϕ, grids, prob.isl);

        rind, mode_lab = PostProcessing.label_mode(ϕ_torfft, torgrids, rmarray, ϕmarray)
        ...
        save_object(dir_base * "/tor_map/efuncs_ft/"*efunc_write, ϕ_torfft)
        mode_count += 1
    ...
    isl_evals = EvalsT(isl_ω, rms, mode_labs)
    save_object(dir_base*"/tor_map/evals.jld2", isl_evals)
end
