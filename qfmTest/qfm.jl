...
#quadratic flux minamised surfaces, eventually to be integrated into MID properly, most likely for fss.

using Roots
using EllipsisNotation
using FFTW
using LaTeXStrings
#using IntervalRootFinding # this option uses a range, could be good as we can set [0, 1], [0, 2π] etc, and is guaranteed (whatever that means)
#however it is different to the scipy one.
using NLsolve # this option uses an initial guess ao may be the same as scipy.

#start with NLsolve.


#we will probably want more than this, this is just the minimum requirements.
#looks like we may need a rho... unsure what the damn hell that even is tho
struct qfm_surface
    scos 
    tsin
    ssin
...
function construct_surfaces(plist, qlist)
    ...
    surfaces = qfm_surface[]

    for i in 1:1:length(plist)
        #obvs will need to pass the other stuff in here somehow.
        scos, tsin, ssin, tcos = action(plist[i], qlist[i])

        push!(surfaces, qfm_surface(scos, tsin, ssin, tcos))
    end
...
#the main function that uses Stuarts method to determine the QFM surfaces. Name subject to change.
#this can only do a single surface at a time...
function action(pp, qq)
    ...
    for jpq in 0:fM-1 #not really sure what we are looping over here.
        ...
        ag(xx) = action_gradient(xx, pp, qq, a, qN, ζ, nlist)
...
function action_gradient(xx, pp, qq, a, qN, ζ, nlist)
...
#this is equivalent of the two waves example, eventually this will just be our usual compute B function!
function compute_B(r, t, z)
